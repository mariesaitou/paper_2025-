...
## ---------------------------------------------------------------------------
## Size-balanced European parental reference analysis
##
## Outputs:
##   snapclust_resampling_balanced_parental_reference_all_runs.tsv
##   snapclust_resampling_balanced_parental_reference_summary_table.tsv
##
## This script repeats the empirical resampling + snapclust analysis after
## subsampling the Atlantic European H. gammarus parental source pool to match
## the American parental source size (n = 38).
## ---------------------------------------------------------------------------
...
get_parental_ids <- function(lobsters) {
  excluded <- c("AmerCook", "Americanus", "HybridX", MED_POPS)
  g_ids_all <- which(pop(lobsters) %in% setdiff(levels(pop(lobsters)), excluded))
  a_ids <- which(pop(lobsters) %in% c("Americanus", "AmerCook"))
  list(g_ids_all = g_ids_all, a_ids = a_ids)
}

make_resampled_classes_balanced <- function(lobsters, n_per_class = 200, seed = 1) {
  set.seed(seed)

  tab_full    <- lobsters@tab
  allele_cols <- colnames(tab_full)

  ids <- get_parental_ids(lobsters)
  g_ids_all <- ids$g_ids_all
  a_ids <- ids$a_ids

  if (length(g_ids_all) < length(a_ids)) {
    stop("European parental source has fewer individuals than the American source.")
  }

  ## Size-balanced source pool for this replicate.
  g_ids <- sample(g_ids_all, length(a_ids), replace = FALSE)
...
run_one <- function(lobsters, n_per_class, seed) {
  message("Balanced reference: N = ", n_per_class, ", seed = ", seed)
  tabs <- make_resampled_classes_balanced(lobsters, n_per_class = n_per_class, seed = seed)
...
reference_counts <- tibble(
  reference = c("American parental source", "Atlantic European parental source", "Balanced European source per replicate"),
  n = c(length(ids$a_ids), length(ids$g_ids_all), length(ids$a_ids))
)
write_tsv(reference_counts, paste0(OUT_PREFIX, "_reference_counts.tsv"))
